...
def store_results(params, net_params, results_dict, config):
    ...
    cur.execute(f'''
        ...
        (params['config'], params['model'], params['dataset'], config['out_dir'], params['seed'], params['epochs'], params['batch_size'], params['init_lr'], params['lr_reduce_factor'], params['lr_schedule_patience'], params['min_lr'], params['weight_decay'], params['print_epoch_interval'], net_params['L'], net_params['hidden_dim'], net_params['out_dim'], net_params['residual'], net_params.get('edge_feat', 'False'), net_params['readout'], net_params.get('n_heads', -1), net_params['in_feat_dropout'], net_params['dropout'], net_params['layer_norm'], net_params['batch_norm'], params['max_time'], net_params['num_train_data'], net_params.get('pos_enc_dim', -1), net_params.get('learned_pos_enc', 'False'), net_params.get('rand_pos_enc', "False"), net_params.get('pos_enc', 'False'), net_params.get('matrix_type', 'N/A'), net_params.get('pow_of_mat', -1), net_params.get('adj_enc', 'False'), net_params.get('num_initials', -1), net_params.get('pagerank', 'False'), final_test_metric, final_train_metric, highest_test_metric, highest_train_metric, epochs_to_convergence, total_time_taken, avg_time_per_epoch, lr, int(net_params['total_param']))
    )
