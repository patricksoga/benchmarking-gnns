#!/bin/bash
#$ -N SAGraphTransformer_SBM_PATTERN_b26-500k-sparse-trials-2
#$ -q gpu
#$ -l gpu_card=1
#$ -t 1-1:1

pos_enc_dim=(0 10)
fname=$(pwd)/b26-500k-sparse-trials-2_${SGE_TASK_ID}_${pos_enc_dim[${SGE_TASK_ID}]}_DEBUG.log
touch $fname
fsync -d 10 $fname &
...
python3 main_SBMs_node_classification.py --config tests/test-configs/SAGraphTransformer_SBMs_SBM_PATTERN_b26-500k-sparse-trials-2.json --job_num ${SGE_TASK_ID} --pos_enc_dim ${pos_enc_dim[${SGE_TASK_ID}]} --log_file $fname
...
#                 'spectral_attn': True,
#                 'wl_pos_enc': False},
#  'out_dir': 'out/SBMs_node_classification_b26-500k-sparse-trials-2',
