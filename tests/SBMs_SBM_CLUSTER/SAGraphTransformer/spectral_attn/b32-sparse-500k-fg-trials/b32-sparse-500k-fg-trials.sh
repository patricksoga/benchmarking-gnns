#!/bin/bash
#$ -N SAGraphTransformer_SBM_CLUSTER_b32-sparse-500k-fg-trials
#$ -q gpu
#$ -l gpu_card=1
#$ -t 1-1:1

pos_enc_dim=(0 10)
fname=$(pwd)/b32-sparse-500k-fg-trials_${SGE_TASK_ID}_${pos_enc_dim[${SGE_TASK_ID}]}_DEBUG.log
touch $fname
fsync -d 10 $fname &
...
python3 main_SBMs_node_classification.py --config tests/test-configs/SAGraphTransformer_SBMs_SBM_CLUSTER_b32-sparse-500k-fg-trials.json --job_num ${SGE_TASK_ID} --pos_enc_dim ${pos_enc_dim[${SGE_TASK_ID}]} --log_file $fname
...
#                 'spectral_attn': True,
#                 'wl_pos_enc': False},
#  'out_dir': 'out/SBMs_node_classification_b32-sparse-500k-fg-trials',
