#!/bin/bash
#$ -N GraphTransformer_OGB_gt_ogb_b1024-baseline-noedge
#$ -q gpu
#$ -l gpu_card=1
#$ -t 1-1:1
...
python3 main_OGB_graph_regression.py --config tests/test-configs/GraphTransformer_OGB_OGB_gt_ogb_b1024-baseline-noedge.json --job_num ${SGE_TASK_ID} --pos_enc_dim ${pos_enc_dim[${SGE_TASK_ID}]} --log_file $fname
...
#python3 configure_tests.py --param_values 20 --job_note gt_ogb_b1024-baseline-noedge --batch_size 1024 --learned_pos_enc False --layer_norm False --batch_norm True --partial_rw_pos_enc False --dataset OGB --config ./test-configs/GraphTransformer_molecules_ZINC_b128-prwpe-noedge.json --hidden_dim 80 --out_dim 80 --seed_array 41 95 22 35 --pos_enc True
